  ...
  const details = referendums.map(
    ({ image, imageHash, status, votedAye, votedNay, votedTotal, votes }) => {
      ...
      if (image && image.proposal) {
        proposalMeta = _extractMetaData(
          image.proposal.registry.findMetaCall(image.proposal.callIndex).meta
        );
          ...
          image.proposal = {
            ...
            args: [
              (args[0].toString().slice(0, 16) +
                "..." +
                args[0].toString().slice(args[0].toString().length - 16)) as any
            ]
          } as any;
        }
      }
...
async function getTreasuryTips(api: ApiPromise) {
  ...
  const optTips = (await (api.query.tips || api.query.treasury).tips.multi(
    tipHashes
  )) as Option<OpenTip>[];
  const tips = optTips
  ...
  return Promise.all(
    tips.map(async (tip: any[]) => {
      ...
      const reason = (await (api.query.tips || api.query.treasury).reasons(
        detail.reason
      )) as Option<Bytes>;
...
async function getDemocracyUnlocks(api: ApiPromise, address: string) {
  ...
  return locks[1]
    .filter(({ isFinished, unlockAt }) => isFinished && locks[0].gt(unlockAt))
