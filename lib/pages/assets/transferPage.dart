...
class _TransferPageState extends State<TransferPage> {
  ...
  Future<String> _getTxFee({bool isXCM = false, bool reload = false}) async {
    ...
    final currencyId = _token == 'vsKSM'
        ? {"VSToken": "KSM", "decimals": 12}
        : _token == 'KUSD'
            ? {"Stable": _token, "decimals": 12}
            : _token == 'vsBOND'
                ? {
  ...
  TxConfirmParams _getTxParams(String chainTo) {
    if (_accountToError == null && _formKey.currentState.validate()) {
      ...
      final currencyId = _token == 'vsKSM'
          ? {"VSToken": "KSM", "decimals": decimals}
          : _token == 'KUSD'
              ? {"Stable": _token, "decimals": decimals}
              : _token == 'vsBOND'
                  ? {
                      "vsBond": ["BNC", 2001, 13, 20],
                      "decimals": decimals
                    }
                  : {"Token": _token, "decimals": decimals};
